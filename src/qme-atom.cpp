...
qme_susceptibility::qme_susceptibility(int theL, int theT, const realnum *theGamma,
...
qme_susceptibility::qme_susceptibility(const qme_susceptibility &from)
    : susceptibility(from) {
  L = from.L;
  T = from.T;
  Gamma = new realnum[L * L];
...
qme_susceptibility::~qme_susceptibility() {
  delete[] Gamma;
  delete[] N0;
  delete[] alpha;
  delete[] omega;
...
void *qme_susceptibility::new_internal_data(realnum *W[NUM_FIELD_COMPONENTS][2],
...
void qme_susceptibility::init_internal_data(realnum *W[NUM_FIELD_COMPONENTS][2], realnum dt,
  ...
  if (!invert(d->GammaInv, L))
    meep::abort("qme_susceptibility: I + Gamma*dt/2 matrix singular");
...
void qme_susceptibility::delete_internal_data(void *data) const {
  if (data) {
    multilevel_data *d = (multilevel_data *)data;
    FOR_COMPONENTS(c) DOCMP2 {
      delete[] d->P[c][cmp];
...
void *qme_susceptibility::copy_internal_data(void *data) const {
  multilevel_data *d = (multilevel_data *)data;
  if (!d) return 0;
...
int qme_susceptibility::num_cinternal_notowned_needed(component c,
                                                             void *P_internal_data) const {
  multilevel_data *d = (multilevel_data *)P_internal_data;
  return d->P[c][0] ? T : 0;
}

realnum *qme_susceptibility::cinternal_notowned_ptr(int inotowned, component c, int cmp,
...
void qme_susceptibility::update_P(realnum *W[NUM_FIELD_COMPONENTS][2],
...
void qme_susceptibility::subtract_P(field_type ft,
